...
place(Block,Board) :-        % we may place it in Rotation 1
  place_1(Block,Board).
place(Block,Board) :-        % we may place it in Rotation 2
  place_2(Block,Board).
place(Block,Board) :-        % we may place it in Rotation 3
  place_3(Block,Board).
place(Block,Board) :-        % we may place it in Rotation 4
  place_4(Block,Board).


place_1(Block,[Row1|[Row2|_Rest]]) :-  % we may place it in the first row
  place_r(Block,Row1), place_c(Block,Row1,Row2).
place_1(Block,[_Row|Rest]) :- % we may place it somewhere else
  place_1(Block,Rest).


place_2(Block,[[Row1|[Row1_1|_]]|[[_Row2|[Row2_1]]_Rest]]) :-  % first row
  place_r(Block,Row1), place_c(Block,Row1_1,Row2_1).
place_2(Block,[_Row|Rest]) :-  % we may place it elsewhere
  place_2(Block,Rest).


place_3(Block,[Row1|[Row2|_Rest]]) :-  % first row
  place_r(Block,Row2), place_c(Block,Row1,Row2).
place_3(Block,[_Row|Rest]) :- % we may place it somewhere else
  place_3(Block,Rest).


place_4(Block,[[_Row1|[Row1_1|_]]|[[Row2|[Row2_1]]_Rest]]) :-  % first row
  place_r(Block,Row2), place_c(Block,Row1_1,Row2_1).
place_4(Block,[_Row|Rest]) :-  % we may place it elsewhere
  place_4(Block,Rest).


% place row
place_r(Block,[Block,Block|_Rest]). % we may place it in the first/current position
place_r(Block,[_Field|Rest]) :-     % we may place it somewhere else
  place_r(Block,Rest).


% place column
place_c(Block,[Block|_R1],[Block|_R2]). % we may place it in the first positions of the rows
